...
class Chains():
    ...
    def _clipDirection( p, box ):
        code = 0
        if p:
            if      p[1] >= box[1][1]:   code |= Chains.TOP
            elif    p[1] <= box[0][1]:   code |= Chains.BOTTOM
            if      p[0] >= box[1][0]:   code |= Chains.RIGHT
            elif    p[0] <= box[0][0]:   code |= Chains.LEFT
        return code
    ...
    def makeImage( chains, box, imageWidth, imageHeight ):
        ...
        #Chains._drawChainsAuto( draw, Chains.bound( chains, box ), offset, scale, imageHeight )
        return pic
    ...
    def deDupe( chains, epsilon=.01 ):
        ...
        for chain in chains:
            ...
            newChain = []
            for x,y in chain:
                if oldX == None or abs(x-oldX) >= epsilon or abs(y-oldY) >= epsilon:
                    newChain.append((x,y))
                oldX, oldY = x, y
            newChains.append( newChain )
        return newChains
