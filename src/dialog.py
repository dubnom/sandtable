...
class DialogFloat( DialogField ):
    def __init__( self, name, prompt, units='', default=0.0, min=None, max=None, randRange=None, format='%g', rbutton=False, slider=True, step=None, rRound=4 ):
        ...
        self.slider = slider if min != None and max != None else False
    ...
    def toForm( self, value ):
        ...
        if self.rbutton:
            ...
            button = ''
        if self.slider:
            slider = '<input name="%s_s" class="floatSlider" type="range" min="%s" max="%s" step="%s" value="%s" oninput="%s.value=%s_s.value">' % (self.name, self.min, self.max, self.step, v, self.name, self.name)
    ...
    def _random( self ):
        ...
        return None if self.min == None or self.max == None else round(random.triangular(self.min,self.max,self.default),self.rRound)
...
class DialogFloats( DialogField ):
    ...
    def _random( self ):
        if self.randRange:
            return [ round( random.triangular( self.randRange[0],self.randRange[1], self.max ), self.rRound) for i in range( random.randint( self.minNums, self.maxNums )) ]
        if self.minNums and self.maxNums:
            return [ round( random.triangular( self.min, self.max ), self.rRound) for i in range( random.randint( self.minNums, self.maxNums )) ]
        return None
...
class DialogInt( DialogField ):
    ...
    def toForm( self, value ):
        ...
        if self.rbutton:
            ...
            button = ''
        if self.slider:
            slider = '<input name="%s_s" class="intSlider" type="range" min="%s" max="%s" value="%s" oninput="%s.value=%s_s.value">' % (self.name, self.min, self.max, v, self.name, self.name)
            sliderInform = 'onchange="%s_s.value=%s.value"' % (self.name, self.name)
        else:
            slider = ''
            sliderInform = ''
    ...
    def _random( self ):
        if self.randRange:
            return int(random.triangular(self.randRange[0],self.randRage[1],self.default))
        return None if self.min == None or self.max == None else int(random.triangular(self.min,self.max,self.default))
